...
const DetailsMoth = ({ invoices }: IProps) => {
    ...
    function getFechamento(invoice: Invoice) {
        if (invoice.bill_status === 'CLOSED') {
            ...
            return <Text style={{ fontFamily: theme.fonts.regular, color: paymentColor }}>{PayStatus[invoice.pay_status]}</Text>
        } 

        return <Text style={{ fontFamily: theme.fonts.regular }}>FECHA EM {getDate(invoice.end_at)}</Text>
    }
    ...
    return (
        <VStack>
            {invoices.map(invoice => (
                <Pressable key={invoice.id} onPress={() => window.alert("detalhamentos do mês")}>
                    <HStack bg={theme.colors.white} p={5} justifyContent={'space-between'} px={10} alignItems={'center'}>
                        <Box>
                            <Text style={{ fontFamily: theme.fonts.semiBold }}>{getMonth(invoice.end_at)}</Text>
                            {getFechamento(invoice)}
                        </Box>
                        <HStack>
                            <Text style={{ fontFamily: theme.fonts.semiBold }}>R$ {invoice.total.toFixed(2)}</Text>
                        </HStack>
                    </HStack>
                </Pressable>
            ))}
